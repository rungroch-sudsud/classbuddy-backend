...
export class VideoService {
    ...
    async createCallRoom(bookingId: string) {
        ...
        if (!teacher) throw new NotFoundException('ไม่พบข้อมูลครูจาก booking นี้');
        ...
        try {
            ...
            console.error('[STREAM VIDEO] Failed to create video call room:', err.message);
            throw new InternalServerErrorException('ไม่สามารถสร้างห้องเรียนได้ในขณะนี้');
        }
    }
    ...
    async joinStudentCall(bookingId: string, userId: string) {
        const booking = await this.bookingModel.findById(bookingId);
        if (!booking) throw new NotFoundException('ไม่พบข้อมูลการจอง');
        if (!booking.callRoomId) throw new BadRequestException('ยังไม่มีห้องเรียนสำหรับการจองนี้');
    ...
    async joinTeacherCall(slotId: string, teacherId: string) {
        ...
        if (!slot.callRoomId) throw new BadRequestException('slot นี้ยังไม่มีห้องเรียน');
